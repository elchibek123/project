...
resource "aws_config_config_rule" "ak3_config_rule" {
  ...
  tags = merge(
    local.common_tags,
    {
      "Name" = "required_tags-shs-ue1-cnfg-001"
    }
  )
...
resource "aws_config_config_rule" "ak3_config_rule_2" {
  ...
  tags = merge(
    local.common_tags,
    {
      "Name" = "sg_ports-shs-ue1-cnfg-001"
    }
  )
...
resource "aws_config_config_rule" "ak3_config_rule_3" {
  ...
  tags = merge(
    local.common_tags,
    {
      "Name" = "s3_public_access-shs-ue1-cnfg-001"
    }
  )
...
resource "aws_config_configuration_recorder" "ak3_config_recorder" {
  name     = "config_recorder-shs-ue1-cnfg-001"
...
resource "aws_iam_role" "ak3_config_role" {
  name               = "config_role-shs-ue1-rl-001"
    ...
    "Statement": [
        {
            ...
            "Resource": [
                "arn:aws:s3:::config-logs-shs-ue1-s3-1/AWSLogs//*"
            ],
            "Condition": {
                "StringLike": {
        ...
        },
        {
            ...
            "Resource": "arn:aws:s3:::config-logs-shs-ue1-s3-001"
        }
    ]
}
POLICY
  ...
  "Statement": [
      {
          "Action": "sns:Publish",
          "Effect": "Allow",
          "Resource": "arn:aws:sns:us-east-1::config-logs-shs-ue1-sns-001"
...
resource "aws_config_delivery_channel" "ak3_delivery_channel" {
  name           = "config_delivery_channel-shs-ue1-cnfg-001"
...
resource "aws_s3_bucket" "ak3_bucket" {
  bucket        = "config-logs-shs-ue1-s3-001"
  force_destroy = true

  tags = merge(
    local.common_tags,
    {
      "Name" = "config-logs-shs-ue1-s3-001"
    }
  )
}
  ...
  policy = <<POLICY
{
    ...
    "Statement": [
        ...
        },
        {
            ...
            "Action": "s3:PutObject",
            "Resource": "arn:aws:s3:::config-logs-shs-ue1-s3-001/AWSLogs//Config/*",
            "Condition": {
                "StringEquals": {
                    "AWS:SourceAccount": "",
                    "s3:x-amz-acl": "bucket-owner-full-control"
...
resource "aws_sns_topic" "ak3_sns_topic" {
  name            = "config-logs-shs-ue1-sns-001"
  delivery_policy = <<EOF
{
  "http": {
